  ...
  if (form) {
    ...
    form.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (res.ok && json.ok) { form.reset(); if (status) { status.textContent = "✅ Got it! B A Roofing will reach out shortly with your free estimate."; status.className = "quote__fine is-ok"; } }
        else fail(json.error || "Something went wrong. Please call (714) 707-8950.");
      } catch { form.reset(); if (status) { status.textContent = "✅ Thanks! Please call (714) 707-8950 to confirm."; status.className = "quote__fine is-ok"; } }
    });
