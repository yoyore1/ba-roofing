// Vercel serverless function: receives free-estimate submissions for B A Roofing.
...
async function deliver(lead) {
  ...
  if (!key || !to) return;
  const from = process.env.LEAD_FROM || "B A Roofing Leads <onboarding@resend.dev>";
  const text = ["New free-estimate request from the website:", "", `Name:     ${lead.name}`, `Phone:    ${lead.phone}`, `Address:  ${lead.address || "—"}`, `Service:  ${lead.service || "—"}`, "", "Message:", lead.message || "—"].join("\n");
  const r = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" }, body: JSON.stringify({ from, to, subject: `🏠 New B A Roofing lead — ${lead.name}`, text }) });
